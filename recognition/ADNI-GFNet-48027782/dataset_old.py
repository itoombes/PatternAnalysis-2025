...
class Adni():
    ...
    def get_test_data(self) -> TensorDataset:
        ...
        # Stack together
        print(control.shape)
        images = torch.cat((control, present))
    ...
    def get_training_and_validation_data(self) -> tuple[TensorDataset, TensorDataset]:
        ...
        for v in present.values():
            training_present.append(torch.from_numpy(v, dtype=torch.float32))
...
def load_image(filepath: str) -> torch.Tensor:
    '''
    Load an image into a PyTorch tensor, with transforms applied
    (i.e., channels x height x width)

    filepath: Path to image
    '''
    transforms = v2.Compose([
        v2.ToImage(), # Convert from PIL image to tensor
...
def load_directory_by_id(directory: str) -> dict[int, np.ndarray]:
    ...
    for key in id_grouped_data:
        id_grouped_data[key] = np.concatenate(id_grouped_data[key], axis=0)

    return id_grouped_data 

if __name__ == "__main__":
    print(load_image(TEST_IMAGE).shape)